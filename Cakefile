remote = 'webfaction:webapps/lewisham'
...
task 'build:stylus', 'build style.css from src/styl', ->
	exec 'stylus -c -u nib -o www/css/ src/styl/style.styl', (err, stdout, stderr) ->
		err && throw err
		exec 'cat www/css/bootstrap.css www/css/style.css > www/css/c.css', (err, stdout, stderr) ->
			err && throw err
